...
public class GoodsInfoFragment extends Fragment implements View.OnClickListener, SlideDetailsLayout.OnSlideDetailsListener {
    ...
    public void onStatucChanged(SlideDetailsLayout.Status status) {
        if (status == SlideDetailsLayout.Status.OPEN) {
            ...
            /**
             * 执行tab上滑动画，商品详情title上滑动画,最终显示title
             */
            ObjectAnimator tabAnimUp = ObjectAnimator.ofFloat(activity.psts_tabs, "translationY", 0f, -activity.psts_tabs.getHeight());
            ObjectAnimator titleAnimUp = ObjectAnimator.ofFloat(activity.tv_title, "translationY", activity.tv_title.getHeight(), 0f);
            AnimatorSet setUp = new AnimatorSet();
            setUp.play(tabAnimUp).with(titleAnimUp);
            setUp.setDuration(360);
            activity.tv_title.setVisibility(View.VISIBLE);
            activity.tv_title.setAlpha(1);
            setUp.start();
...
//            activity.psts_tabs.setVisibility(View.GONE);
            ...
            /**
             * 执行tab下滑动画，商品详情title下滑动画，最终显示tab
             */
            ObjectAnimator tabAnimDown = ObjectAnimator.ofFloat(activity.psts_tabs, "translationY", -activity.psts_tabs.getHeight(), 0f);
            ObjectAnimator titleAnimDown = ObjectAnimator.ofFloat(activity.tv_title, "translationY", 0f, activity.tv_title.getHeight());

            AnimatorSet setDown = new AnimatorSet();
            setDown.play(tabAnimDown).with(titleAnimDown);
            setDown.setDuration(360);
            activity.tv_title.setVisibility(View.VISIBLE);
            activity.tv_title.setAlpha(1);
            setDown.start();
